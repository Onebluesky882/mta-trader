...
const optimizeRouter = new Hono<{ Bindings: Bindings; Variables: Variables }>()
...
optimizeRouter.post('/', async (c) => {
  ...
  if (
    typeof result.totalTrades !== 'number' || typeof result.winRate !== 'number' ||
    typeof result.totalProfit !== 'number' || typeof result.maxDrawdown !== 'number'
  ) {
    return c.json({ error: 'Invalid parameters', code: 'INVALID_PARAMS' }, 400)
  }
